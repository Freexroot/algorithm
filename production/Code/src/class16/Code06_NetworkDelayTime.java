...
//times{[起点，终点，边长],...},n是节点数,k是起始点.
//求初始点到任意点中最长的边和
public class Code06_NetworkDelayTime {
    //自己实现的
    public int networkDelayTime(int[][] times, int n, int k) {
        Code06_Dijkstra dijkstra = new Code06_Dijkstra();
        GraphGenerator graphGenerator = new GraphGenerator();
        Graph graph = graphGenerator.createGraph(times);
        HashMap<Node, Integer> nodeIntegerHashMap = null;
        for (Node value : graph.nodes.values()) {
            if (value.value == k) {
                nodeIntegerHashMap = dijkstra.dijkstra2(value, n);
                break;
            }
        }
        int minimum = Integer.MAX_VALUE;
        for (Node node : nodeIntegerHashMap.keySet()) {
            minimum = Math.min(minimum, node.value);
        }
        return minimum;
    }
